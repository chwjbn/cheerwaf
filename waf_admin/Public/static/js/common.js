...
$(function(){
    ...
    //自动生成面包屑导航
    var nav_obj = $(".location").find("span");

    if(nav_obj.size()>0)
    {

        var flag = $.inArray($(nav_obj[1]).text(),['首页'])<0;
        if(flag)
        {
        ...
        if(nav_obj[1])
        {
                ...
                $(nav_obj[1]).text($(".list-group:visible",parent.document).find("h2").text());
        ...
        if(nav_obj[2])
        {
                ...
                $(nav_obj[2]).text($(".list-group:visible", parent.document).find('.selected').parents("ul").prev().find("span:first").text());
        ...
        if(nav_obj[3])
        {
                ...
                $(nav_obj[3]).text($(".list-group:visible", parent.document).find('.selected').find('span:first').text());
    ...
    //询问框
    $(document).on("click","#txtKeywords",function()
    {
        $(this).next().click();
    })
    ...
    //询问框
    $(document).on("click",".need_del_",function()
    {
        var that = $(this);
        ...
        var layer_index = layer.confirm(txt, {
            btn: ['确认','取消'] //按钮
        }, function(){
            that.next().click();
    ...
    //发送ajax请求
    $(document).on("click",".end_ajax_",function()
    {

        var that= $(this);
        var href = that.data("href");
        var callback = 'doSearch';
        ...
        $.post(href,data,function(data){
            if(data.error_code == 0){
                layer.msg(data.msg, {icon: 1},eval(callback));
            }else{
                layer.msg(data.msg, {icon: 0},eval(callback));
            }
        },'json')
    ...
    //layer
    $(document).on("click",'[layer_url]',function()
    {
        var url=$(this).attr('layer_url');
        var title=$(this).attr('layer_title');
        var h=$(this).attr('layer_h');
        var w=$(this).attr('layer_w');
        size = ['90%', '90%'];
    ...
    $(document).mousedown(function(e)
    {
        if(e.which == 2)
        {
            var x = e.clientX+"px";
            var y = e.clientY+"px";
            var layer_index = layer.confirm('是否要在这里添加备注', {
                btn: ['确认','取消'] //按钮
            }, function(){
                $("body").append("<span class='mark_span'>备注</span>");
                $(".mark_span").css("position","absolute").css("top",y).css("left",x).css("font-size",'10px').css("color",'#6c7d85').css("cursor",'pointer').addClass("show_mark").removeClass("mark_span").data("top", e.clientY).data("left", e.clientX).data("url",window.location.href);
            }, function(){
    ...
    $(document).on("keyup","#txtKeywords",function(e)
    {
        if(e.which == 13)
        {
            $(this).next()[0].click();
    ...
    $(document).on("click",".show_mark",function(e)
    {
         var that = $(this);
         var id = that.data("id");
         var data = that.data();
         if(!id&&data.url)
         {
             $.post('/api/add_mark_callback',data,function(data){
                 if(data.error_code == 0){
                     layer.msg(data.msg, {icon: 1},eval(callback));
                 }else{
                     layer.msg(data.msg, {icon: 0},eval(callback));
         ...
         if(id)
         {
             ...
             layer.open({
                 ...
                 title: '备注详情',
                 area:size ,
                 content: '/api/show_mark_pop?id='+id
             });
         }
    });

    $(document).on("mousewheel",".show_mark",function(e)
    {
        ...
        if(e.ctrlKey)
        {
            ...
            $(this).css("top",top+"px").addClass("change",true).data("top",top);
        }
        ...
        if(e.altKey)
        {
            ...
            $(this).css("left",left+"px").data("change",true).data("left",left);
        }
